...
export default class AuthController {
    ...
    public async register({ request, auth }: HttpContextContract) {
      const productSchema = schema.create({
        email: schema.string({ trim: true },[rules.required(),rules.minLength(3),rules.email(),rules
        ...
        contactNumber: schema.string.optional({trim:true},[rules.mobile(),rules.unique({table:'users',column:'contact_number'})]),
