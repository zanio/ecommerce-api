...
export default class ProductsController {
    ...
    public async store ({ auth, request, response}: HttpContextContract) {
        const productSchema = schema.create({
            ...
            productSubCategoryId: schema.number([rules.required(), rules.exists({table:'product_sub_categories',column:'id'})]),
            price: schema.number([rules.required(), ]),
            productCategoryId: schema.number([rules.required(), rules.exists({table:'product_categories',column:'id'})]),
          })
